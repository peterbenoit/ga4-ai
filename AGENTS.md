...
- **Pete is not a GA4 expert. Assume zero prior knowledge of GA4 concepts.** He's the front-end dev building this tool for VA staff who will ask him analytics questions he can't personally answer — that's the whole reason this extension exists. Never introduce a GA4-specific concept (funnels, key events, dimensions vs. metrics, channel groups, whatever) without a one-line plain-English explanation of what it means and why it matters, the first time it comes up in a session. Don't assume he remembers GA4 terminology from a past session just because it's written in a requirements doc — re-explain it inline when it resurfaces. This applies to UI copy too: status/error messages shown in the extension should be readable by someone who doesn't know GA4 jargon, not just by the agent that wrote them.
