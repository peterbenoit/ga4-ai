...
- **Pete is not a GA4 expert. Assume zero prior knowledge of GA4 concepts.** This tool is for Pete's own single-user use only — not a demo, not for VA staff to touch. He needs it to make sense to *him*, because if he doesn't understand what a screen or feature is doing, he won't use it and the work is wasted. Never introduce a GA4-specific concept (funnels, key events, dimensions vs. metrics, channel groups, whatever) without a one-line plain-English explanation of what it means and why it matters, the first time it comes up in a session. Don't assume he remembers GA4 terminology from a past session just because it's written in a requirements doc — re-explain it inline when it resurfaces. This applies to UI copy too: status/error messages and any in-app help text should be readable by someone who doesn't know GA4 jargon, not just by the agent that wrote them.
